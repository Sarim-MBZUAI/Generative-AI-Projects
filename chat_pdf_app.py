...
def get_pdf_text(pdf_docs):
    ...
    text = ""
    for pdf in pdf_docs:
        pdf_file_stream = io.BytesIO(pdf.getvalue())  # Convert bytes to a file-like object
        pdf_reader = PdfReader(pdf_file_stream)
        for page in pdf_reader.pages:
            text += page.extract_text() or ""  # Safely handle None returns from extract_text
    return text
...
def main():
        ...
        with st.sidebar:
             ...
             pdf_docs = st.file_uploader("Upload your PDF Files and click the submit button")
